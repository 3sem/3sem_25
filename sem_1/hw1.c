...
void seq_pipe(struct CmdStruct* cmd);

struct CmdStruct* cmds_ctor();
struct CmdStruct* get_cmds();
void cmds_dtor(struct CmdStruct* cmds);
...
int main()
{
    while (1)
    {
        struct CmdStruct* cmds = get_cmds();
...
struct CmdStruct* cmds_ctor()
{
    struct CmdStruct* cmds = (struct CmdStruct*)calloc(MAX_CMDS_AMT, sizeof(struct CmdStruct));
...
struct CmdStruct* get_cmds()
{
    ...
    struct CmdStruct* cmds = cmds_ctor();
